...
def main(args):
    ...
    while t < args.num_trials:
        ...
        hand_fail = False
        front_gesture_1, side_gesture_1 = get_visual_observation(env)
        ...
        if hand_fail:
            env.close()
            continue
        front_gesture_2, side_gesture_2 = get_visual_observation(env)
        env.switch_phase()
        ...
        policy = LearnedPolicy(agent, transform, front_gesture_1, side_gesture_1, front_gesture_2, side_gesture_2, device)
        front_image, side_image = get_visual_observation(env)
        action = policy.react((front_image, side_image))
        env.step(action)
...
def get_visual_observation(env):
    front_image, side_image = env.render_offscreen()
    front_image = Image.fromarray(front_image, mode="RGBA").convert("RGB")
    side_image = Image.fromarray(side_image, mode="RGBA").convert("RGB")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model", type=str, choices=["AttentionActionPredictor"], default="AttentionActionPredictor")
